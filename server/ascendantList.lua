...
local function sacramentMinor()
  ...
  WaitFor("targetAlly", function(data)
    local selectedUnit, tileRef = unpack(data)
    -- TODO: first, we remove Chosen status from any previous Chosen
    -- then, we change the units name to '**[name]**'
    client:send("modifyUnitTable", {selectedUnit, tileRef, 'name', '**'..selectedUnit.name..'**'})
...
local function sacramentVictory(playerNumber)
  if not Gamestate['Chosen'..playerNumber] then
    return false
  end
...
ascendantList[1] = {
  ...
  majorText='Resurrect any Unit that has died this game and place them on a chosen Tile.',
